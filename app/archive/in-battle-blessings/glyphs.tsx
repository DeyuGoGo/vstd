import type { BlessingGlyphKind } from './data/blessings';

interface BlessingGlyphProps {
  kind: BlessingGlyphKind;
  tint: string;
}

export const BlessingGlyph = ({ kind, tint }: BlessingGlyphProps) => {
    ...
    return (
      <svg width="92" height="92" viewBox="-50 -50 100 100" style={{ filter: glow }}>
        <g>
          ...
          <path
            d="M-30 -30 L-3 -3 M30 -30 L3 -3 M-30 30 L-3 3 M30 30 L3 3"
            stroke={tint}
            strokeWidth="3"
            strokeLinecap="round"
          />
        ...
        {[-22, 0, 22].map((dx, i) => {
          ...
          return (
            <g key={i} transform={`translate(${dx} ${ofs})`}>
              <path
                d="M0 -32 L-3 -22 L0 -16 L3 -22 Z M0 -16 L0 26"
                stroke={tint}
                strokeWidth="3"
                strokeLinecap="round"
                fill={tint}
  ...
  if (kind === 'heart')
    return (
      <svg width="96" height="92" viewBox="-50 -48 100 96" style={{ filter: glow }}>
        <path
          d="M0 32 C-44 4 -36 -36 -14 -36 C-4 -36 0 -28 0 -22 C0 -28 4 -36 14 -36 C36 -36 44 4 0 32 Z"
          fill={`${tint}aa`}
          stroke={tint}
          strokeWidth="2.5"
        />
        <path
          d="M-10 -20 Q-4 -28 0 -20"
          stroke="#fff"
          strokeWidth="2.5"
          fill="none"
          strokeLinecap="round"
      ...
      <svg width="100" height="80" viewBox="-50 -40 100 80" style={{ filter: glow }}>
        {[0.4, 0.7, 1].map((s, i) => (
          <path
            ...
            fill={i === 2 ? '#fff' : tint}
            opacity={i === 2 ? 1 : 0.4 + 0.3 * i}
          />
        ))}
        <path
          d="M-32 -8 L-22 -2 M-32 8 L-22 2 M32 -8 L22 -2 M32 8 L22 2"
          stroke={tint}
          strokeWidth="2"
          strokeLinecap="round"
        />
